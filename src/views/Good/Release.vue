	...
	<el-card class="box-card">
		...
		<el-form ref="form" :model="form" label-width="120px" label-position="left" :rules="rules">
			...
			<el-form-item label="商品轮播图" prop="slider">
				<el-upload action="/api/upload/slider" :on-success="handleSuccess" list-type="picture-card" :on-preview="handlePictureCardPreview"
		...
		methods: {
			...
			handleSubmit(form) {
				this.$refs[form].validate(async (valid) => {
					if (valid) {
						let { status } = await Good.release({ ...this.form });
						if (status) {
